...
import heartHandIcon from "@/assets/hero-hand-hearts-icon.png";

type Stat = {
  icon: LucideIcon | string;
  secondaryIcon?: LucideIcon | string;
  label: string;
  value: string;
  color?: string;
};
...
const Hero = () => {
  const stats: Stat[] = [
    ...
    {
      icon: heartHandIcon,
      label: "Patients Treated by Our Clinicians",
      value: "10K+",
      color: "text-rose-400",
    },
  ...
  return (
    <section className="relative min-h-[90svh] md:min-h-screen flex items-center justify-center overflow-hidden bg-white md:bg-transparent">
      {/* Background */}
      <picture>
        <source srcSet='https://images.pexels.com/photos/4101143/pexels-photo-4101143.jpeg' media="(min-width: 768px)" />
        <img
          src={heroBgMobile}
          alt=""
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-start">
            ...
            <div className="relative space-y-8 p-6 sm:p-8 rounded-2xl animate-fade-in-up">
              <div className="space-y-5">
                ...
                <p className="text-lg md:text-xl text-white/90 leading-relaxed max-w-2xl">
                  Professional psychiatric and psychological services providing
                  compassionate, evidence-based assessments and medical
                  treatments for Children, Adolescents, Adults, and Families
                  across the UAE.
                </p>
              </div>
              ...
              <div className="grid sm:grid-cols-2 gap-3">
                {features.map((feature, index) => (
                  ...
                  >
                    ...
                    <span className="text-white/90 text-sm leading-6">
                      {feature}
                    </span>
                  </div>
                ))}
          ...
          <div className="space-y-6 animate-fade-in">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                    ...
                    <div className="flex gap-2 justify-center sm:justify-start">
                      {typeof stat.icon === "string" ? (
                        <img src={stat.icon} alt="" className="h-16 w-16" />
                      ) : (
                        <stat.icon className={`h-8 w-8 ${stat.color}`} />
                      )}

                      {stat.secondaryIcon &&
                        (typeof stat.secondaryIcon === "string" ? (
                          <img src={stat.secondaryIcon} alt="" className="h-12 w-11" />
                        ) : (
                          <stat.secondaryIcon className={`h-8 w-8 ${stat.color}`} />
                        ))}
                    </div>
            ...
            <div className="bg-black/30 backdrop-blur-md rounded-xl p-6 border border-white/20">
              <h3 className="text-white font-semibold mb-4 tracking-tight">
                Need Assistance?
              </h3>
              <div className="space-y-3">
                <div className="flex items-center space-x-3">
                  ...
                  <span className="text-white/90">
                    Confidential enquiries (routine & urgent)
                  </span>
                </div>
                <div className="flex items-center space-x-3">
